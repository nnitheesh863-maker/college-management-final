...
const roleConfig = {
  student: {
    ...
    theme: {
      label: 'border-blue-500/40', activeBg: 'from-blue-600/30 to-cyan-600/20',
      avatar: 'from-blue-600 to-cyan-600', navActive: 'border-blue-500/30',
    }
  },
  teacher: {
    ...
    theme: {
      label: 'border-purple-500/40', activeBg: 'from-purple-600/30 to-pink-600/20',
      avatar: 'from-purple-600 to-pink-600', navActive: 'border-purple-500/30',
    }
  },
  principal: {
    gradient: 'from-amber-500 via-yellow-500 to-purple-600',
    icon: '🏫', label: 'Principal Portal',
    accent: 'border-amber-500/40',
    sidebarAccent: 'bg-gradient-to-r from-amber-600/30 to-purple-600/20',
    glowColor: 'rgba(251,191,36,0.3)',
    theme: {
      label: 'border-amber-500/40', activeBg: 'from-amber-600/30 to-purple-600/20',
      avatar: 'from-amber-600 to-purple-600', navActive: 'border-amber-500/30',
    }
  }
};
  ...
  return (
    <div className="min-h-screen bg-[#020617] relative overflow-hidden">
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute -top-40 -left-40 w-[600px] h-[600px] rounded-full blur-[120px] opacity-30 float" style={{ background: `radial-gradient(circle, ${config.glowColor} 0%, transparent 70%)` }} />
        <div className="absolute -bottom-40 -right-40 w-[600px] h-[600px] rounded-full blur-[120px] opacity-30 float-delayed" style={{ background: `radial-gradient(circle, ${config.glowColor} 0%, transparent 70%)` }} />
      </div>
      ...
      <div className="relative z-10 flex min-h-screen">
        ...
        >
          <div className="p-4 border-b border-white/10 flex items-center gap-3 flex-shrink-0">
            <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${theme.avatar} flex items-center justify-center text-white font-bold text-lg flex-shrink-0 shadow-lg`}>
          ...
          <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
            {navItems.map((item) => (
              <motion.button
                key={item.id} whileHover={{ x: sidebarOpen ? 4 : 0 }} whileTap={{ scale: 0.97 }}
                onClick={() => setActiveSection(item.id)}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-300 text-sm relative
              ...
              >
                {activeSection === item.id && (
                  <motion.div layoutId={`${role}ActiveTab`}
          ...
          <div className="p-3 border-t border-white/10 flex-shrink-0 space-y-1">
            <button onClick={() => setSidebarOpen(!sidebarOpen)}
              ...
              <span className={`transition-transform duration-300 ${sidebarOpen ? '' : 'rotate-180'}`}>{sidebarOpen ? '◀' : '▶'}</span>
            </button>
        ...
        <main className={`flex-1 transition-all duration-300 min-h-screen ${sidebarOpen ? 'ml-60' : 'ml-[72px]'}`}>
          <header className="sticky top-0 z-20 px-6 py-3 flex items-center justify-between border-b border-white/5"
            style={{ background: 'rgba(2,6,23,0.8)', backdropFilter: 'blur(20px)' }}
          >
            <div className="flex items-center gap-4">
              <button onClick={() => setSidebarOpen(!sidebarOpen)} className="text-slate-500 hover:text-white transition-colors text-xl">☰</button>
              <h2 className="text-slate-400 text-sm hidden md:block capitalize">{activeSection}</h2>
            </div>
            <div className="flex items-center gap-3">
              <div className="text-slate-400 text-xs">{user?.name || 'User'}</div>
              <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${theme.avatar} flex items-center justify-center text-white text-sm font-bold shadow-lg`}>
                {(user?.name || 'U')[0]}
              </div>
            </div>
          </header>
