  ...
  try {
    ...
    if (leave) {
      ...
      createAndEmitNotification({
        userId: leave.userId,
        message: `Your leave request${leaveUser ? ` (${leaveUser.name})` : ''} was ${req.body.status}`,
        type: req.body.status === 'approved' ? 'success' : 'warning',
        category: 'leave',
        link: leave.role === 'student' ? '/student-dashboard' : '/teacher-dashboard',
      });
    }
    res.json(leave);
  } catch (err) {
...
router.post('/announcements', async (req, res) => {
  try {
    ...
    for (const r of recipients) {
      createAndEmitNotification({
        userId: r._id,
        message: `📢 ${ann.title}${ann.description ? ': ' + ann.description.substring(0, 120) : ''}`,
        type: 'info',
        category: 'announcement',
        link: r.role === 'student' ? '/student-dashboard' : r.role === 'teacher' ? '/teacher-dashboard' : '/principal-dashboard',
      });
    }
