...
async function seed() {
  await connectDB();
  await Promise.all([
    User.deleteMany({}), Student.deleteMany({}), Teacher.deleteMany({}), Parent.deleteMany({}),
  ...
  // Parent
  const parentUser = await User.create({ name: 'Michael Johnson', email: 'parent@demo.edu', password: 'password123', role: 'parent' });
  ...
  console.log(`   Parent: parent@demo.edu / password123`);
  process.exit(0);
}
